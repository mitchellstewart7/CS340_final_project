module.exports = function(){
    ...
    function getTitles(res, mysql, context, complete){
        mysql.pool.query("SELECT title, payScale FROM Titles", function(error, results, fields){
            if(error){
    ...
    router.post('/', function(req, res){
        ...
        if (req.body.title == '' || req.body.payScale == '')
        {
            res.redirect('/titles');
        }
        else
        {
            var mysql = req.app.get('mysql');
            var sql = "INSERT INTO Titles (title, payScale) VALUES (?,?)";
            var inserts = [req.body.title, req.body.payScale];
    ...
    router.put('/:title', function(req, res){
        ...
        var sql = "UPDATE Titles SET payScale=? WHERE title=?";
        var inserts = [req.body.payScale, req.params.title];
        sql = mysql.pool.query(sql,inserts,function(error, results, fields){
            if(error){
                console.log(error)
                res.write(JSON.stringify(error));
    ...
    router.delete('/:title', function(req, res){
        var mysql = req.app.get('mysql');
        var sql = "DELETE FROM Titles WHERE title = ?";
        var inserts = [req.params.title];
        sql = mysql.pool.query(sql, inserts, function(error, results, fields){
            if(error){
                console.log(error)
                res.write(JSON.stringify(error));
